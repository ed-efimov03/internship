...
def minable_required(func):
    def wrapper(crypto):
        if not crypto.minable:
            ...
            return None  # Возвращаем None, если криптовалюта не поддается майнингу
        return func(crypto)
    return wrapper

# Базовый класс криптовалют
class Cryptocurrency:
    def __init__(self, name, symbol, minable, rate_to_usd, anonymous):
        self.name = name  # Название криптовалюты
        self.symbol = symbol  # Символ криптовалюты
        self.minable = minable  # Флаг возможности майнинга
        self.rate_to_usd = rate_to_usd  # Курс к доллару США
        self.anonymous = anonymous  # Флаг анонимности транзакций

    def mining_reward(self):
        return None  # По умолчанию награды за майнинг нет
    
    def __str__(self):
        return f"{self.name} ({self.symbol})"  # Форматированный вывод криптовалюты

# Класс Nano, поддерживает блок-решетку
class Nano(Cryptocurrency):
    def __init__(self, block_lattice=False, rate_to_usd=5, anonymous=True):
        ...
        self.block_lattice = block_lattice  # Поддержка блок-решетки

    @minable_required
    def mining_reward(self):
        return 0.02 if self.block_lattice else 0.01  # Зависящая от block_lattice награда

# Класс Iota, использует технологию Tangle
class Iota(Cryptocurrency):
    def __init__(self, tangle=False, rate_to_usd=0.3, anonymous=False):
        ...
        self.tangle = tangle  # Поддержка Tangle

    @minable_required
    def mining_reward(self):
        return 0.001 if self.tangle else 0.002  # Награда зависит от использования Tangle

# Класс Stellar, не поддерживает майнинг
class Stellar(Cryptocurrency):
    def __init__(self, distributed=False, rate_to_usd=0.1, anonymous=True):
        ...
        self.distributed = distributed  # Поддержка распределенной сети

    def mining_reward(self):
        return None  # Stellar не поддерживает майнинг
...
# Вывод информации о криптовалютах и награды за майнинг (если применимо)
